...
join_dis <- read_csv(here("data","discharge_zscore.csv")) |>
  inner_join(
...
shapes <- c(21, 22, 23, 24)   # filled shapes
...
selected_sites <- sf_dis |>
  filter(site_name %in% c("B9", "R28.1", "JA.25.India.1990", "JAVA12.1985"))
...
world_map_plot <- world_map |>
  ...
  geom_point(
    ...
    alpha = 0.7,
    stroke = 1,
    color = "yellow",     # yellow outline only for selected sites
